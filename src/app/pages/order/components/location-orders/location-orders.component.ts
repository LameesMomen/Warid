...
@Component({
  selector: 'app-location-orders',
  templateUrl: './location-orders.component.html',
  styleUrls: ['./location-orders.component.css'],
})
export class LocationOrdersComponent {
  @Input() public allLocationOrders: any;
  page: number = 1;
  ...
  delete(item: any) {
    this.toasters.confirmationToaster({
      ...
      confirmFunc: () => {
        ...
        this.http
          .deleteLocation(`/locationmanager/admin/locations/${item.id}`)
          .subscribe(
            (res) => {
